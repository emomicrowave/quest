...
def _calendar_print(tasks: List[Tuple[Arrow, TaskDB]], date: Arrow) -> None:
    ...
    today = lambda d: "bold yellow" if d.isocalendar() == date.isocalendar() else "white" if d.month == date.month else "bright_black"
    format = lambda date, db: Text("\n").join([Text(f"{date.day}          ", style=today(date)), format_tdb(db, False, False)])
